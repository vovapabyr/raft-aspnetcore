...
public class CandidateNodeState : NodeState
{
    ...
    public CandidateNodeState(int currentTerm, string? votedFor, IList<LogEntry> log, int commitLength, string? currentLeader, HashSet<string> votesReceived) : base(currentTerm, votedFor, log, commitLength, currentLeader)
    {
    ...
    public override LeaderNodeState CopyAsLeader(List<string> nodesIds) => new LeaderNodeState(_currentTerm, _votedFor, new List<LogEntry>(_log), _commitLength, _currentLeader, nodesIds); 
}